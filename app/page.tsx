...
export default function Home() {
  ...
  const isWorker = selected && account.toLowerCase() === selected.worker.toLowerCase();
  const isParty = selected && [selected.worker.toLowerCase(), selected.client.toLowerCase()].includes(account.toLowerCase());
  const finalizableAt = selected?.reviewed_at ? new Date((selected.reviewed_at + 86400) * 1000) : null;
  ...
  return <main>
    ...
    {selected && <div className="modal-backdrop" onClick={() => setSelected(null)}><article className="modal" role="dialog" aria-modal="true" aria-labelledby="job-dialog-title" onClick={e => e.stopPropagation()}><button className="close" aria-label="Close job details" onClick={() => setSelected(null)}>×</button><p className="eyebrow">JOB {selected.id} · {selected.status}</p><h2 id="job-dialog-title">{selected.title}</h2><p>{selected.brief}</p><dl><div><dt>Escrow</dt><dd>{Number(BigInt(selected.amount || "0")) / 1e18} GEN</dd></div><div><dt>Verdict</dt><dd>{selected.verdict || "Pending"}</dd></div><div><dt>Worker release</dt><dd>{selected.release_bps / 100}%</dd></div><div><dt>Confidence</dt><dd>{selected.confidence}%</dd></div></dl>{selected.reason && <blockquote>{selected.reason}</blockquote>}
      {selected.status === "OPEN" && isWorker && <form onSubmit={e => { e.preventDefault(); const url = new FormData(e.currentTarget).get("evidence"); void mutate(selected, "submit_delivery", [selected.id, url], j => j.status === "SUBMITTED"); }}><input name="evidence" type="url" required placeholder="https://… delivery evidence"/><button className="primary">Submit delivery</button></form>}
      {selected.status === "OPEN" && !isWorker && <p className="role-note">Only the assigned worker can submit delivery evidence.</p>}
      ...
      {selected.status === "REVIEWED" && <>{isParty && <form onSubmit={e => { e.preventDefault(); const reason = new FormData(e.currentTarget).get("reason"); void mutate(selected, "challenge", [selected.id, reason], j => j.status === "CHALLENGED" && policyBoundToExecution(j)); }}><textarea name="reason" minLength={20} required placeholder="Appeal with a concrete discrepancy…"/><button className="secondary">Challenge once</button></form>}<p className="role-note">Finalizable after {finalizableAt?.toLocaleString()}.</p><button className="primary full" onClick={() => void mutate(selected, "finalize", [selected.id], j => j.status === "FINALIZED" && policyBoundToExecution(j))}>Finalize after 24h</button></>}
      {selected.status === "CHALLENGED" && <><p className="role-note">Appeal finalizable after {finalizableAt?.toLocaleString()}.</p><button className="primary full" onClick={() => void mutate(selected, "finalize", [selected.id], j => j.status === "FINALIZED" && policyBoundToExecution(j))}>Finalize appeal after 24h</button></>}
    </article></div>}
